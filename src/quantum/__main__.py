...
def main(argv=None) -> int:
    ...
    # Subsample once by INDEX (kernel cost is quadratic); stratify on binary
    # label. Indexing X, y_bin, and y_multi with the same sample_idx keeps all
    # three row-aligned regardless of which task is requested.
    if len(X_full) > args.max_samples:
        all_idx = np.arange(len(X_full))
        sample_idx, _ = train_test_split(
            all_idx, train_size=args.max_samples, stratify=y_bin_full,
