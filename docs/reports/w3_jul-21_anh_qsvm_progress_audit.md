...
| **iqp-vs-angle on the 15-class task**                                                                                                               | **RESOLVED** (2026-07-27)  | iqp wins decisively on 16-class at n=1000 (unanimous 5/5 at nc=6) but not on the 15-class reframe (8/15 over nc = 1, 3, 6). Since 15-class is now the primary multiclass task, this needs a sweep past nc=6 and a direct held-out macro-F1 comparison rather than inner-CV win-counting. See §2, Days 1–3. **Resolution:** [`w4_jul-27_encoding_verdict.md`](w4_jul-27_encoding_verdict.md) replaces win-counting with direct held-out macro-F1 across nc = 1, 3, 6, 8. iqp beats angle at every nc tested, but the margin (+0.001 to +0.012) is thin and smaller than either encoding's own fold-to-fold std — a real but weak edge, not the large clean separation seen on EMBER 15-class. Win-counting is retired as a methodology for this project. |
